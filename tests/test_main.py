...
async def test_embed_external_api_failure(test_client: AsyncClient):
    ...
    with (
        ...
        assert (
            ...
            == "An error occurred while computing or saving vector representation."
        )
...
async def test_rag_endpoint_success(test_client: AsyncClient):
    ...
    mock_completion_data.choices = [
        SimpleNamespace(
            message=SimpleNamespace(content="You need high hydration for sourdough.")
        )
    ]
    ...
    with (
        ...
        assert (
            response_json["sources"][0]["text"]
            == "Sourdough bread needs high hydration."
        )
